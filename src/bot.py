...
class BetBot:
    def __init__(self, config: dict):#, Updater: object):
        ...
        self.fast = WebDriverWait(self.browser, timeout=5)
        self.wait = WebDriverWait(self.browser, timeout=30)

        # Carregar
        # self.browser.get(url)

        back = self.browser.current_window_handle
        # get url
        self.browser.get('https://bet365.com')
        self.browser.switch_to.target(self.browser.targets[0]["targetId"])
        time.sleep(5)
        self.browser.switch_to.target(back)

        # self.browser.get('https://www.google.com/')
        print('Esperando tela de login...')
